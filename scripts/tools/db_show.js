#!/usr/bin/env node
// db_show.js — 展示单篇文章完整记录（MySQL；--include-content 输出正文）
// 用法:
...
async function showOne(article, includeContent) {
  ...
  const classification = (await my.query("SELECT content_type, business_category, topic_cluster, confidence, reason, classifier_type, created_at FROM content_classifications WHERE entity_type = 'articles' AND entity_id = ? ORDER BY created_at DESC LIMIT 1", [article.id]))[0];

  const out = {
    article: {
      id: article.id, title: article.title, slug: article.slug, status: article.status,
      contentType: article.content_type, businessCategory: article.business_category, topicCluster: article.topic_cluster,
    ...
    classification: classification ? { ...classification, confidence: classification.confidence != null ? Number(classification.confidence) : null, created_at: String(classification.created_at) } : null,

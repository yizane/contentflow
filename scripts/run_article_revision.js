#!/usr/bin/env node
// run_article_revision.js — 单独执行文章修订（DB-only 薄壳；依赖最新版本的 source_resolution_json）
const my = require('./mysql_lib');
const { reviseArticleWithResolution } = require('./pipeline_lib');
...
async function main() {
  ...
  try {
    ...
    if (!resolution) {
      console.log(JSON.stringify({ ok: false, error: '最新版本无 source_resolution_json，先运行 npm run run:source-resolution' }, null, 2));
      process.exitCode = 1;
      return;
    }

#!/usr/bin/env node
// jobs_create.js — 从 topic_candidates 选高分主题写入 article_jobs 表（DB-only）
// 用法: npm run jobs:create -- --limit 1 --min-score 80 [--category X] [--strategy balanced] [--dry-run]
...
async function main() {
  ...
  try {
    ...
    if (candidates.length === 0) {
      console.log(JSON.stringify({ ok: false, error: `没有符合条件的候选主题（score>=${args.minScore}）`, hint: '先运行 npm run topics:generate 或降低 --min-score' }, null, 2));
      process.exitCode = 1;
      return;
    }

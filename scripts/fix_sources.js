#!/usr/bin/env node
// fix_sources.js — needs_fact_sources → ready_for_review 修订闭环（DB-only）
// 流程: 来源补全 → 文章修订（新 version）→ 修订稿重新核查 → 状态推进，全部写 MySQL
// 用法:
//   npm run fix:sources -- --limit 1
//   npm run fix:sources -- --article-id <id> [--force]
//   npm run fix:sources -- --slug <slug>
const my = require('./mysql_lib');
const { resolveSourcesForArticle, reviseArticleWithResolution, factCheckArticle } = require('./pipeline_lib');

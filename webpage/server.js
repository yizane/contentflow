#!/usr/bin/env node
// server.js — ContentFlow 监控台服务（public/ 前端 + 只读 API + 受控运行/终审操作）
// 只读 MySQL 为主；不暴露 .env/密码/RDS 地址；默认不返回完整 prompt/raw_response（仅摘要）。
// 用法: cd webpage && npm start   或   PORT=5178 npm start
...
const PY_WORKDIR = path.join(ROOT, 'workflow_py');
...
function dt(v) {
  // DB DATETIME 经 mysql2（timezone:'Z'）解析为正确的 UTC 时刻；统一返回 UTC ISO，前端按本地显示。
  if (!v) return null;
  const d = v instanceof Date ? v : new Date(v);
  return Number.isNaN(d.getTime()) ? String(v) : d.toISOString();
}

function spawnContentflow(command, args = [], options = {}) {
  return spawn('uv', ['run', 'contentflow', ...command, ...args], {
...
async function runControlStart(body) {
  ...
  // 后台启动 Python workflow；控制逻辑与归档由 contentflow engine daily 执行，立即返回 accepted
  const child = spawnContentflow(['engine', 'daily'], ['--mode', mode, '--daily-key', dailyKey, '--actor', actor, '--trigger-source', 'viewer'], {
...
// ---------- 单步重跑（Web 控制台）----------
// 白名单：step key → Python workflow CLI。绝不透传任意命令/参数。
const STEP_CMDS = {
  collect:   { command: ['sources', 'collect'],        label: '采集来源', engineRunId: true },
  classify:  { command: ['content', 'classify'],       label: '分类回填（主链路外）', engineRunId: true },
  topics:    { command: ['topics', 'generate'],        label: '生成主题池', engineRunId: true },
  select:    { command: ['topics', 'select'],          label: '选题入选（排队待写）', engineRunId: true },
  generate:  { command: ['articles', 'generate'],      label: '生成文章初稿', engineRunId: true },
  factcheck: { command: ['articles', 'factcheck'],     label: '事实核查门禁', engineRunId: true },
  sourcesfix:{ command: ['sources', 'fix'],            label: '补来源与修订', engineRunId: true },
  channels:  { command: ['channels', 'generate'],      label: '渠道改写', engineRunId: true },
  score:     { command: ['score', 'seo-geo'],          label: 'SEO/GEO 评分', engineRunId: true },
  quality:   { command: ['score', 'article-quality'],  label: '质量主评分', engineRunId: true },
  report:    { command: ['engine', 'report'],          label: '生产报告' },
};
...
async function startStep(step) {
  ...
  // 把单步重跑挂到当天 active daily run（Python CLI 通过 --engine-run-id 接收），
  ...
  const args = cmd.engineRunId && env.ENGINE_RUN_ID ? ['--engine-run-id', env.ENGINE_RUN_ID] : [];
  const child = spawnContentflow(cmd.command, args, { env });
  const push = (chunk) => {
...
// Python workflow 的过程日志写 MySQL workflow_events（不再写 logs/ 文件），
// 抽屉里把任务启动后的事件也实时带上，否则 AI 调用的几分钟里看起来像卡死。
// 时区：DB DATETIME 是 UTC 墙钟。WHERE 比较要用 UTC 串；展示给人看用本地时区。
function utcSqlDt(d) {
  return d.toISOString().slice(0, 19).replace('T', ' ');
}
function localDisplayDt(v) {
  if (!v) return '-';
  const d = v instanceof Date ? v : new Date(v);
  const p = (n) => String(n).padStart(2, '0');
  return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}:${p(d.getSeconds())}`;
}
async function processEventsSince(engineRunId, startedAtIso, max = 150) {
  const since = new Date(startedAtIso ? new Date(startedAtIso).getTime() - 2000 : Date.now() - 3600_000);
  let sql = 'SELECT level, event_type, message, created_at FROM workflow_events WHERE created_at >= ?';
  const params = [utcSqlDt(since)];
  if (engineRunId) { sql += ' AND engine_run_id = ?'; params.push(engineRunId); }
  sql += ` ORDER BY created_at ASC LIMIT ${max}`;
  try {
    const rows = await my.query(sql, params);
    return rows.map((r) => `[${localDisplayDt(r.created_at)}] [${(r.level || 'info').toUpperCase()}] ${r.event_type}: ${r.message || ''}`.slice(0, 400));
  } catch (_) { return []; }
}
  ...
  try {
    ...
    if ((m = p.match(/^\/api\/step\/log\/([a-z]+)$/))) {
      ...
      return j ? json(res, 200, {
        ...
        startedAt: j.startedAt, log: j.log, fileLog: await processEventsSince(j.engineRunId, j.startedAt),
    ...
    if (p === '/api/topic-auditions/run' && req.method === 'POST') {
      ...
      const child = spawnContentflow(['topic', 'audition'], ['--rounds', String(rounds), '--limit', String(lim), '--json'], {
        detached: true, stdio: 'ignore', env: process.env,
      });
      child.unref();
    ...
    if (abs.startsWith(VIEWER_DIR + path.sep) && fs.existsSync(abs) && fs.statSync(abs).isFile()) {

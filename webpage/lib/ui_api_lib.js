...
// 业务语义命名：topics_select=选题入选/排队、articles_generate=生成初稿、articles_factcheck=事实核查门禁。
const STEP_KEY_MAP = {
  sources_collect: 'collect',
  topics_generate: 'topics',
  topics_select: 'tasks',
  articles_generate: 'generate',
  articles_factcheck: 'factcheck',
...
function stepSummary(key, status, out, errorMessage) {
  ...
  if (key === 'collect') {
    ...
    // topics_select：选中 N 题、排队 M 篇待写（article_writing_tasks）
    if (o.writingTaskCount != null || o.selectedCount != null) {
      const n = o.writingTaskCount != null ? o.writingTaskCount : o.selectedCount;
      summary = `入选 ${n} 篇待写${o.deferredCount ? `，延期 ${o.deferredCount}` : ''}`;
      metrics.push(['待写', n], ['延期', o.deferredCount || 0]);
    }
  } else if (key === 'generate') {
    if (o.succeeded != null) {
      summary = `生成 ${o.succeeded} 篇文章${o.failed ? `，失败 ${o.failed}` : ''}`;
      metrics.push(['生成', o.succeeded], ['失败', o.failed || 0]);
    }
  } else if (key === 'factcheck') {
    if (o.succeeded != null) {
      summary = `核查 ${o.succeeded} 篇${o.failed ? `，失败 ${o.failed}` : ''}`;
      metrics.push(['核查', o.succeeded], ['失败', o.failed || 0]);
    }

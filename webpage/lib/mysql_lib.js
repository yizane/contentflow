...
// 复用仓库根 .env（与 workflow_py 共用一套环境变量），不依赖启动 cwd
...
function configFromEnv() {
  ...
  return {
    ...
    charset: 'utf8mb4',
    // workflow（Python）把 DATETIME 以 UTC 墙钟写入；按 UTC 解析才能得到正确的绝对时刻，
    // 前端再用本地 getter 显示成业务时区（Asia/Shanghai）。不设则 mysql2 默认按本地解析，整体偏 8 小时。
    timezone: 'Z',
  };
}
...
// model_runs 记录（返回 id，供 content_classifications 等关联）
async function recordModelRun(fields) {
  const id = makeId('mrun');
